...
export default function () {
  ...
  addButton.addEventListener("click", () => {
    ...
    const formattedDueDate = taskDueDate.toLocaleDateString("en-US", {
      weekday: "long",
      year: "numeric",
      month: "long",
      day: "numeric",
    });
    if (taskName !== "" || formattedDueDate !== "Invalid Date") {
      try {
  ...
  document.addEventListener("click", (e) => {
    if (
      ...
      !addTaskButton.contains(e.target)
    ) {
      form.reset();
      formContainer.classList.remove("show");
    }
  ...
  addFileButton.addEventListener("click", () => {
    ...
    div.innerHTML =
      "<input type='text' name='addFileInput' id='addFileInput' placeholder='Enter File Name'> <button type='submit' id='appendFile'>Add</button> <button id='cancelFile'>Cancel</button>";
